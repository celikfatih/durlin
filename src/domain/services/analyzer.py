...
class DiffAnalyzerService:
    ...
    def analyze_and_comment(self, issue_key: str, git_reference: typing.Optional[str] = None, dry_run: bool = False) -> str:
        """
        Executes the analysis pipeline.
        
        Args:
            issue_key: The target Jira issue key (e.g., PROJ-123)
            git_reference: Optional Git reference to diff (e.g., HEAD~1). If None, discovers via Jira.
            dry_run: If True, returns the comment but does not post to Jira.
        ...
        if git_reference:
            logger.info(f"Fetching diff for explicit reference: {git_reference}")
            diff_content = self.git.get_diff(git_reference)
        else:
